...
# def make_a_pic(num=2, url='img/img.jpg', text="123456789012345"):
#     # try:
#         sleep(1)
#         img = Image.open(url)
#         height = img.size[1]
...
#         v = variants[num]
#         new_txt_bg = txt_bg.resize((round(height * v["bg_w"]), round(height * v["bg_h"])))
#         new_logo = logo.resize((round(height * 0.2), round(height * 0.2)))
#
#         c = Canvas(root, width=height, height=height)
#         tk_im = ImageTk.PhotoImage(img)
#         tk_bg = ImageTk.PhotoImage(new_txt_bg)
#         tk_logo = ImageTk.PhotoImage(new_logo)
#
#         c.create_image(height // 2, height // 2, image=tk_im)
#         c.create_image(height * v['bg_x'], height * v['bg_y'], image=tk_bg)
#         c.create_image(height * 0.1, height * 0.1, image=tk_logo)
#         c.create_text(height * v['bg_x'], height * v['bg_y'], text=text, fill="#db9200", font=("Arial Black", round(height * 0.077)))
#
#         c.pack()
#
#         c.update()
#         def save():
#             ps = c.postscript(colormode="color", pageheight=4980, pagewidth=5000)
